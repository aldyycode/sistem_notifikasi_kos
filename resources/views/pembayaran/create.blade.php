...
<div class="p-6 max-w-4xl mx-auto">
    ...
    {{-- HEADER HALAMAN --}}
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-slate-900">Tambah Pembayaran</h2>
        <p class="text-sm text-slate-500 mt-1">Buat tagihan pembayaran baru dan tentukan batas waktu jatuh tempo.</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 sm:p-8">
            ...
            <form action="{{ route('pembayaran.store') }}" method="POST">
                ...
                <div class="mb-8">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Detail Tagihan</h3>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Penghuni <span class="text-red-500">*</span></label>
                            <select name="id_penghuni" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-800 focus:border-slate-800 transition-colors bg-white" required>
                                <option value="">-- Pilih Penghuni --</option>
                                @foreach($penghunis as $p)
                                    <option value="{{ $p->id_penghuni }}" {{ old('id_penghuni') == $p->id_penghuni ? 'selected' : '' }}>
                                        {{ $p->nama_penghuni }} - Kamar {{ $p->kamar?->nomor_kamar ?? '-' }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Jumlah Bayar (Rp) <span class="text-red-500">*</span></label>
                            <input type="number" 
                                   name="jumlah_bayar" 
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100">
                    ...
                    <a href="{{ route('pembayaran.index') }}" class="inline-flex items-center justify-center px-6 py-2.5 border border-slate-300 shadow-sm text-sm font-medium rounded-lg text-slate-700 bg-white hover:bg-slate-50 transition-colors w-full sm:w-auto">
